    ...
    public function check() {
        ...
        while(!$queue->isEmpty()) {
            ...
            foreach($postset as $id => $edge) {
                ...
                // get the set of places that are marked unbounded by the preset
                $unboundedByPreset = $this->unboundedFromPreset($edge->to);
                // create the largest possible set of coverable places
                $maxCover = $coverable->union($unboundedByPreset);
                // does the new marking cover at least all the places covered by the preset
                $validFromPreset = Utils\SetUtils::isSubset($unboundedByPreset, $unbounded);
                // does the new marking cover all possible coverables
                $validFromCoverable = Utils\SetUtils::isSubset($unbounded, $maxCover);
                // a marking is valid if we were able to find a substitute, when it covers
                // the preset, and covers any new coverables
                $validMarking = !is_null($petrinetMarking) && $validFromPreset && $validFromCoverable;
